...
Dev slave also receives the plan file path via a **standing order** (as today).
...
The MVP loop is **synchronous**: while a `jonAsk*` loop runs the chat input is disabled and only Stop
(the existing abort path, [ADR-0018](0018-abort-path-parity.md)) is available.
